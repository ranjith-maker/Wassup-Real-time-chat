...
export default function ProfileDetails() {
    ...
    // Assuming you store the active chat user context in your Redux store under chat.selectedUser
    const selectedUser = useSelector((state) => state.chat?.selectedUser);

    // Hardcoded fallback data for safety if a user reloads the page directly
    const user = {
        name: selectedUser?.name || "Rohith",
        profilePic: selectedUser?.profilePic || `https://api.dicebear.com/7.x/adventurer/svg?seed=${selectedUser?.name}`,
        bio: selectedUser?.bio || "Hey there! I am using this application to chat, build cool systems, and stay connected.",
    };
    ...
    return (
        <div className="min-h-screen flex flex-col bg-[#0880bc] text-slate-100 antialiased relative overflow-hidden">
            ...
            <main className="w-full max-w-2xl mx-auto px-4 mt-12 flex-1 flex flex-col items-center">
                ...
                {/* Profile Dashboard Cards Section */}
                <section className="w-full mt-10 space-y-4">
                    ...
                    {/* Meta Info Placeholder Card (Matches Personal Profile Consistency) */}
